        ...
        implements Cloneable
{
    ...
    /**
     * Hard version of findMin, includes deleted nodes in the search,
     * used for collectGarbage method to find min for hard deletion
     * Uses hard traversal rather than soft traversal
     * @param root of tree
     * @return the value of the minimum element
     */
    private E findMinHard(LazySTNode root) {
        if (root == null) { // null check
            return null;
        }
        // Inner class to traverse tree and find minimum element
    ...
    /**
     * Hard version of findMax, includes deleted nodes in the search,
     * used for collectGarbage method to find max for hard deletion
     * Uses hard traversal rather than soft traversal
     * @param root of a tree
     * @return the value of the maximum element
     */
    private E findMaxHard(LazySTNode root) {
        if (root == null) { // null check
            return null;
        }
        class MaxFinder extends Traverser<E> {
        ...
        // create instance of inner class and traverse tree
        MaxFinder maxFinder = new MaxFinder();
        traverseHard(maxFinder); // the difference of soft and hard!!!!
        if (maxFinder.getMax() == null) { // null check for max
            throw new NoSuchElementException("No maximum element found.");
        }
        return maxFinder.getMax();
    ...
    protected LazySTNode remove(LazySTNode root, E x)
    {
        ...
        if ( compareResult < 0 ) // if x is less than current node data, go left
            ...
            if (!root.deleted) { // if node is not already marked as deleted
                root.deleted = true; // mark node as deleted
                mSize--; // decrement soft size
            }
        }
        // found the node
        return root;
    ...
    protected LazySTNode removeHard(LazySTNode root, E x)
    {
        ...
        {
            root.data = findMin(root.rtChild);
    ...
    public boolean collectGarbage() {
        int prevSize = mSizeHard; // store old size to compare later
        mRoot = collectGarbage(mRoot);
        return prevSize != mSizeHard; // calculate number of nodes collected
    }
    ...
    private LazySTNode collectGarbage(LazySTNode root) {
        ...
        if (root.deleted) {
            return removeHard(root, root.data); // remove node from tree
        }
        return root;
    }
    /**
